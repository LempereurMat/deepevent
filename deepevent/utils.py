...
def saveLFS(acq,eventLFS,ff,freq,SubjectValue):
...
def saveRFS(acq,eventRFS,ff,freq,SubjectValue):
...
def saveLFO(acq,eventLFO,ff,freq,SubjectValue):
...
def saveRFO(acq,eventRFO,ff,freq,SubjectValue):
...
def loadmodel():
	json_file = open(Path(__file__).parent / 'data\DeepEventModel.json','r')
	loaded_model_json = json_file.read()
	json_file.close()
	return loaded_model_json

def loadweight(loaded_model_json):
	if not os.path.exists(Path(__file__).parent / 'data\DeepEventWeight.h5'):
		gdd.download_file_from_google_drive(file_id='1w7LmluewzGbx5EK4V2TIXsUhE953-Oh3',dest_path=Path(__file__).parent / 'data\DeepEventWeight.h5',unzip=False)

	model = model_from_json(loaded_model_json)
	model.load_weights(Path(__file__).parent / 'data\DeepEventWeight.h5')
	return model
'''
